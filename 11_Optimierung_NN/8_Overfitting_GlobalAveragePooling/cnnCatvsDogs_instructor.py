...
def build_model(img_shape: Tuple[int, int, int], num_classes: int, dr_rate: int) -> Sequential:
    ...
    catdog_model.add(Conv2D(filters=2, kernel_size=3, padding="same"))
    catdog_model.add(BatchNormalization())
...
if __name__ == "__main__":
    ...
    for rate in dr_rates:
        model_name = f"catvsdog_4block_GlobalAverage_BN_dr_{rate}"

        model = build_model(data.img_shape, data.num_classes, rate)
