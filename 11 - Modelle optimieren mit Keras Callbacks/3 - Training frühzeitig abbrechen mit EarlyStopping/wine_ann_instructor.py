...
def train():
    ...
    checkpoint = ModelCheckpoint(filepath="model.{epoch:02d}-{val_loss:.2f}.h5", monitor="val_loss", mode="min", verbose=1, save_best_only=False, save_weights_only=False)
    earlystop = EarlyStopping(monitor="val_loss", patience=1, mode="min", verbose=1)
