...
def train():
    ...
    chk_clb = ModelCheckpoint(model_name, monitor='val_acc', save_best_only=True)
    reduce_lr_clb = ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.1), patience=5, min_lr=min_learning_rate)
    # Fit the model on the batches generated by datagen.flow().
    history = model.fit_generator(datagen.flow(x_train, y_train, 
                        batch_size=batch_size),
                        validation_data=(x_test, y_test),
                        steps_per_epoch=data_amount // batch_size,
                        epochs=epochs, verbose=1, workers=4,
                        callbacks=[chk_clb, lr_clb, reduce_lr_clb])
